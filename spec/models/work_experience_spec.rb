...
RSpec.describe WorkExperience, type: :model do
  ...
  it 'belongs to a user' do
    work_experience = create(:work_experience, user: user) # Assuming you have a factory for WorkExperience
  ...
  it 'is not valid without a company' do
    work_experience = build(:work_experience, user: user, company: nil)
    expect(work_experience).not_to be_valid
  end

  it 'is not valid without a start date' do
    work_experience = build(:work_experience, user: user, start_date: nil)
    expect(work_experience).not_to be_valid
  end

  it 'is not valid without a job title' do
    work_experience = build(:work_experience, user: user, job_title: nil)
    expect(work_experience).not_to be_valid
  end

  it 'is not valid without a location' do
    work_experience = build(:work_experience, user: user, location: nil)
    expect(work_experience).not_to be_valid
  end

  it 'is not valid without an employment type' do
    work_experience = build(:work_experience, user: user, employment_type: nil)
    expect(work_experience).not_to be_valid
  end

  it 'is not valid with an invalid employment type' do
    work_experience = build(:work_experience, user: user, employment_type: 'Invalid Type')
    expect(work_experience).not_to be_valid
  end

  it 'is not valid without a location type' do
    work_experience = build(:work_experience, user: user, location_type: nil)
    expect(work_experience).not_to be_valid
  end

  it 'is not valid with an invalid location type' do
    work_experience = build(:work_experience, user: user, location_type: 'Invalid Type')
    expect(work_experience).not_to be_valid
  end

  it 'is not valid if end date is greater than start date' do
    work_experience = build(:work_experience, user: user, start_date: Date.today, end_date: Date.yesterday)
    expect(work_experience).not_to be_valid
  end

  it 'validates presence of end date when not currently working here' do
    work_experience = build(:work_experience, user: user, end_date: nil, currently_working_here: false)
  ...
  it 'is not valid if end date is present and currently_working_here is true' do
    work_experience = build(:work_experience, user: user, start_date: Date.today, end_date: Date.tomorrow, currently_working_here: true)

    expect(work_experience).not_to be_valid
  end

  it 'is not valid if end date is missing when currently_working_here is true' do
    work_experience = build(:work_experience, user: user, start_date: Date.today, currently_working_here: true)
    expect(work_experience).not_to be_valid
  end
  ...
  it 'calculates job duration correctly if end_date is present' do
    ...
    work_experience = build(:work_experience, user: user, start_date: start_date, end_date: end_date)
  ...
  it 'calculates job duration when currently working here' do
    start_date = Date.new(2020, 1, 1)
    work_experience = build(:work_experience, user: user, start_date: start_date, currently_working_here: true)
  ...
  it 'formats company with employment type' do
    work_experience = build(:work_experience, user: user, company: 'ABC Inc', employment_type: 'Full-time')
    expect(work_experience.company_with_employment_type).to eq("ABC Inc (Full-time)")
  end

  it 'formats job location' do
    work_experience = build(:work_experience, user: user, location: 'New York', location_type: 'On-site')
